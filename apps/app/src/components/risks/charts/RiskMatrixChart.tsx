...
const getProbabilityLevels = (t: (content: string) => string) => [t('Very Likely'), t('Likely'), t('Possible'), t('Unlikely'), t('Very Unlikely')];
const probabilityNumbers = ['5', '4', '3', '2', '1'];
const getProbabilityLabels = (t: (content: string) => string) => [
  t('Very Likely (5)'),
  t('Likely (4)'),
  t('Possible (3)'),
  t('Unlikely (2)'),
...
const getImpactLevels = (t: (content: string) => string) => [t('Insignificant'), t('Minor'), t('Moderate'), t('Major'), t('Severe')];
            ...
            {hasChanges && (
              ...
              >
                <Button onClick={handleSave} variant="default" disabled={loading}>
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <T>Save</T>}
                </Button>
              </motion.div>
            )}
          </AnimatePresence>
