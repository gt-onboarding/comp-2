...
export function IntegrationsCard({
  ...
  const getUTCMidnightMessage = (nextRunAt: Date): string => {
    ...
    if (diffInHours === 0) {
      return t('Runs in {minutes} minute{plural}', {
    ...
    if (diffInMinutes === 0) {
      return t('Runs in {hours} hour{plural}', {
        hours: diffInHours,
        plural: diffInHours !== 1 ? 's' : ''
      });
    }

    return t('Runs in {hours} hour{hourPlural} and {minutes} minute{minutePlural}', {
      hours: diffInHours,
      hourPlural: diffInHours !== 1 ? 's' : '',
      minutes: diffInMinutes,
      minutePlural: diffInMinutes !== 1 ? 's' : ''
    });
  };
