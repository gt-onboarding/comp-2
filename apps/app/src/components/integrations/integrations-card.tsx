...
import { T, useGT, Branch, Plural } from 'gt-next';
...
export function IntegrationsCard({
  ...
  const deleteIntegrationConnection = useAction(deleteIntegrationConnectionAction, {
    onSuccess: () => {
      toast.success(t('Integration disconnected successfully'));
    },
    onError: () => {
      toast.error(t('Failed to disconnect integration'));
    },
  });
  ...
  const handleConnect = async () => {
    try {
      ...
      toast.error(t('Failed to connect integration'));
    } finally {
      setLoading(false);
      setIsSaving(false);
    }
  ...
  const getUTCMidnightMessage = (nextRunAt: Date): string => {
    ...
    if (diffInHours <= 0 && diffInMinutes <= 0) {
      return t('Running soon');
    }

    if (diffInHours === 0) {
      return diffInMinutes === 1 
        ? t('Runs in {minutes} minute', { minutes: diffInMinutes })
        : t('Runs in {minutes} minutes', { minutes: diffInMinutes });
    }

    if (diffInMinutes === 0) {
      return diffInHours === 1
        ? t('Runs in {hours} hour', { hours: diffInHours })
        : t('Runs in {hours} hours', { hours: diffInHours });
    }

    const hoursText = diffInHours === 1 ? t('hour') : t('hours');
    const minutesText = diffInMinutes === 1 ? t('minute') : t('minutes');
    return t('Runs in {hours} {hoursText} and {minutes} {minutesText}', {
      hours: diffInHours,
      hoursText,
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            {nextRunAt ? (
                              ...
                              <p className="text-muted-foreground text-xs">
                                {lastRunAt ? t('Calculating...') : t('Will run at the next midnight UTC')}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        {(!installedSettings?.api_key || isEditingApiKey) && (
                          <div className="space-y-4">
                            <div className="space-y-2">
                              ...
                              >
                                {isEditingApiKey ? t('Update API Key') : t('Enter API Key')}
                              </label>
                              <Input
                                id={`${id}-api-key`}
                                type="password"
                                placeholder={t('Enter your Deel API key')}
                                value={apiKeyInput}
