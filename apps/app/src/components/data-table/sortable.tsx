  ...
  const announcements: Announcements = React.useMemo(
    () => ({
      onDragStart({ active }) {
        ...
        return t('Grabbed sortable item "{item}". Current position is {position} of {total}. Use arrow keys to move, space to drop.', {
          item: activeValue,
      ...
      onDragOver({ active, over }) {
        if (over) {
          ...
          const activeValue = active.id.toString();
          return t('Sortable item "{item}" moved {direction} to position {position} of {total}.', {
            item: activeValue,
      ...
      onDragEnd({ active, over }) {
        ...
        if (over) {
          const overIndex = over.data.current?.sortable.index ?? 0;
          return t('Sortable item "{item}" dropped at position {position} of {total}.', {
            item: activeValue,
            position: overIndex + 1,
            total: value.length
          });
      ...
      onDragCancel({ active }) {
        const activeIndex = active.data.current?.sortable.index ?? 0;
        const activeValue = active.id.toString();
        return t('Sorting cancelled. Sortable item "{item}" returned to position {position} of {total}.', {
          item: activeValue,
      ...
      onDragMove({ active, over }) {
        if (over) {
          ...
          const moveDirection = overIndex > activeIndex ? 'down' : 'up';
          const activeValue = active.id.toString();
          return t('Sortable item "{item}" is moving {direction} to position {position} of {total}.', {
            item: activeValue,
            direction: moveDirection,
            position: overIndex + 1,
            total: value.length
