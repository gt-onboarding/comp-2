  ...
  return (
    <div className={cn('flex items-center justify-between px-2 py-4', className)} {...props}>
      <div className="text-muted-foreground flex items-center gap-4 text-sm">
        <span className="hidden sm:inline"><T><Num>{table.getCoreRowModel().rows.length}</Num> items</T></span>
      ...
      <div className="flex items-center gap-1">
        <span className="text-muted-foreground mr-2 hidden text-sm sm:inline">
          <T>Page <Num>{table.getState().pagination.pageIndex + 1}</Num> of <Num>{table.getPageCount()}</Num></T>
        </span>
        <Button
          variant="outline"
          size="sm"
