...
const getWorkItems = (t: (content: string, options?: InlineTranslationOptions) => string): WorkItem[] => [
  {
    id: '1',
      ...
      <div className="space-y-2">
        ...
        <T>
          <p className="text-xs text-muted-foreground/70">
            <Var>{completedCount}</Var> of <Var>{workItems.length}</Var> tasks completed • Estimated time remaining:{' '}
            <Var>{overallProgress < 10
              ? t('6-7 min')
              : overallProgress < 20
                ? t('5-6 min')
                : overallProgress < 40
                    ...
                    {isProcessing && (
                      <div className="space-y-2 pl-5">
                        ...
                        <div className="space-y-1">
                          ...
                          >
                            {Math.round(item.progress)}% {t('complete')}
                            {isStuck && t(' - Finalizing...')}
                          </p>
                        </div>
                      </div>
                    )}
