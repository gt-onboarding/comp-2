      ...
      const processFile = (file: File) => {
        return new Promise<void>((resolve) => {
          ...
          if (file.size > MAX_FILE_SIZE_BYTES) {
            toast.error(t('File "{name}" exceeds the {size}MB limit.', { name: file.name, size: MAX_FILE_SIZE_MB }));
          ...
          reader.onloadend = async () => {
            ...
            if (!base64Data) {
              toast.error(t('Failed to read file data for {name}', { name: file.name }));
              return resolve();
            }
            ...
            if (error) {
              ...
              toast.error(t('Failed to upload "{name}": {error}', { name: file.name, error }));
              ...
              toast.success(t('File "{name}" ready for attachment.', { name: data?.name ?? 'unknown' }));
            } else {
              console.error('Upload succeeded but missing data:', data);
              toast.error(t('Failed to process "{name}" after upload.', { name: file.name }));
            }
            resolve();
          };
          reader.onerror = () => {
            toast.error(t('Error reading file: {name}', { name: file.name }));
            resolve();
          };
              ...
              <div className="pt-6">
                {isEditing ? (
                  <div className="flex flex-col gap-2">
                    <T><Label className="block text-xs font-medium">Attachments</Label></T>
                    <div className="flex flex-col gap-2">
