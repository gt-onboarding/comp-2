...
export const createCompanyDetailsSchema = async () => {
  const t = await getGT();
  return z.object({
...
export const createSteps = async (): Promise<Step[]> => {
  const t = await getGT();
  return [
    {
      key: 'frameworkIds',
    ...
    },
    {
      ...
      options: ['AWS', t('Google Cloud'), t('Microsoft Azure'), 'Heroku', 'Vercel', t('Other')],
    },
    {
      key: 'dataTypes',
...
export const createWelcomeText = async () => {
  const t = await getGT();
  return [
