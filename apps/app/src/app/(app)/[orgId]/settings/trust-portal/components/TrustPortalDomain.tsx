...
export function TrustPortalDomain({
  ...
  const form = useForm<z.infer<ReturnType<typeof getTrustPortalDomainSchema>>>({
    resolver: zodResolver(getTrustPortalDomainSchema(t)),
    defaultValues: {
      domain: initialDomain || '',
    },
  });

  const onSubmit = async (data: z.infer<ReturnType<typeof getTrustPortalDomainSchema>>) => {
