  ...
  return (
    <DialogContent className="max-w-md">
      <DialogHeader className="space-y-2">
        <T><DialogTitle className="text-base font-medium">Add Frameworks</DialogTitle></T>
        <DialogDescription className="text-muted-foreground text-sm">
          {availableFrameworks.length > 0
            ? t('Select the compliance frameworks to add to your organization.')
            : t('No new frameworks are available to add at this time.')}
        </DialogDescription>
      </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <T><FormLabel className="text-sm font-normal">Available Frameworks</FormLabel></T>
                  <FormControl>
      ...
      {!isExecuting && availableFrameworks.length === 0 && (
        <div className="py-6 text-center">
          <T><div className="text-muted-foreground text-sm">
      ...
      {isExecuting && (
        <div className="flex items-center justify-center py-8">
          ...
          <T><span className="text-muted-foreground ml-3 text-sm">Adding frameworks...</span></T>
        </div>
      )}
    </DialogContent>
  );
